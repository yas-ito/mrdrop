...
- **Apple の返事待ち**（通れば「リリース」を押した時点で公開）
  - 🆕 **2026-09-07 21:17 に催促を送りました**（`ios/APPSTORE-review-followup.md`）。
    9/4 12:29 の返信から**中3日 動きが無かった**ため、①まだ情報が要るか
    ②再提出すべきか の2点を聞いています。**ステータスは「却下済み
    （2.1.0 Performance: App Completeness）」のまま**で、
    **「App Reviewに再提出」ボタンはグレーで押せません**
