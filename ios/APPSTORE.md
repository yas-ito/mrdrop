...
> ✅ **2026-09-03 に提出済み（1.0.0 (1)・審査待ち）。App ID 6808082392。**
> 以下は次の版を出すときの手順として残す。App Store Connect の入力欄は **form_input では React に反映されない＝クリックしてキー入力**、
> セレクトは JS で value を入れて change を投げる。スクリーンショットは **6.5 インチ（1284×2778）**。
> 「コンテンツ配信権」と審査連絡先の電話番号が無いと提出できない。

**方針（2026-09-03・本人の決定）**: iPhone アプリは **App Store で無料**、お金は **PC 側（BOOTH で有料）** で取る。
Windows 版は無署名のまま。

🔴 **アプリの中に「買う」ボタンやリンクを置かない。**代わりに、**入れた人が使えるようになる案内は必ず書く**
（PC 側が要ること・どこで手に入るか）。売るためではなく、**使えないまま放置しないため**。
参考: App Store の規約 3.1.1 が、アプリ内から外部の
